    ...
    // Prisma Known Request Errors
        ...
        if (err.code === 'P2002') {
            ...
            error = {
                ...
                field,
                constraint: 'unique',
                details: `A ${modelName?.toLowerCase() || 'record'} with this ${field} already exists`,
                ...(target && { target }),
            };
            ...
            error = {
                model: modelName,
                ...fieldInfo,
                details: getP2025Details(modelName, fieldInfo.field),
...
function extractFieldFromP2025Error(err, req) {
    ...
    if (urlParams.length > 0) {
        const primaryKey = urlParams.find((param) => ['id', 'userId', 'productId', 'postId'].includes(param)) ||
            urlParams[0];
        return {
            field: primaryKey,
...
function getP2025Details(modelName, field) {
