...
const getDonorDemographics = async (_filters = {}) => {
    const [total, donors, byCity] = await Promise.all([
        ...
        prisma_1.prisma.profile.groupBy({
            by: ['city'],
            _count: { id: true },
...
const getDonationTrends = async (filters = {}) => {
...
const getRequestAnalytics = async (filters = {}) => {
    const { period = 'month' } = filters;
    const now = new Date();
    let startDate = new Date();
    switch (period) {
    ...
    const avgFulfillmentRate = fulfilledStats.length > 0
        ? (fulfilledStats.reduce((sum, r) => sum + (r.fulfilledUnits / r.unitsRequired), 0) / fulfilledStats.length) * 100
        : 0;
    return {
        period,
        dateRange: { startDate, endDate: now },
...
const getDashboardMetrics = async () => {
    const [totalDonors, totalDonations, totalRequests, fulfillmentRate, topBloodGroups] = await Promise.all([
        prisma_1.prisma.user.count({ where: { role: 'USER' } }),
